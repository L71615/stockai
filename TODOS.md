...
### T8: quickAddDCA 测试覆盖
- **What:** 为快捷定投录入功能补测试：NAV 获取成功/失败、金额记忆有/无、双击去重、localStorage 不可用
- **Why:** 5 条明确路径，复用了现有 add-lot 端点，需验证 fallback 逻辑和去重防护
- **Pros:** 测试规范已定（5 条），pytest 就绪后立即可写
- **Cons:** 依赖 T1 (pytest 基础设施)
- **Context:** 由 /plan-ceo-review 产出。测试: test_quick_add_dca_success, test_quick_add_dca_nav_fail, test_quick_add_dca_no_amount, test_quick_add_dca_duplicate, test_quick_add_dca_storage_fallback
- **Depends on:** T1
- **Added:** 2026-05-31 by /plan-ceo-review
