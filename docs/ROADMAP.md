...
>
> ⚡ **当前重点：策略可编程 → 回测验证 → 纪律执行 → 复盘改进 全链路**
> 详见 `memory/stockai-one-month-plan.md`
...
| 开源融合 | 🟡 20% (F05 35% + F08 + F13 done — Alpha158 待补完 103/158) |
...
- [x] **F05**: Alpha158 因子体系 (基础) ✅ 55/158 因子 (10大类, 103个待移植) — `factor_service.py`
- [ ] **F05b**: Alpha158 因子补完 — 分 3 批移植剩余 ~103 个因子

**Alpha158 缺失因子清单（按 Qlib 10大类）：**

| 类别 | 当前 | 目标 | 缺失 |
|------|------|------|------|
| K线衍生 (KBar) | ~15 | ~40 | ~25 |
| 滚动窗口 (Rolling) | ~8 | ~30 | ~22 — 偏度/峰度/分位数 |
| 指数加权 (EMA) | ~5 | ~15 | ~10 — EMA交叉比值 |
| 动量 (Momentum) | ~9 | ~23 | ~14 — 威廉/CCI/多周期 |
| 波动率 (Volatility) | ~8 | ~10 | ~2 |
| 资金流 (MoneyFlow) | ~3 | ~10 | ~7 — MFI/AD 等 |
| 相关性 (Correlation) | 0 | ~5 | ~5 — 个股-指数动态相关 |
| 宏观/市场 (Macro) | 0 | ~5 | ~5 — 行业/市场相对收益 |
| 价格形态 (Price) | ~5 | ~10 | ~5 |
| 成交量 (Volume) | ~6 | ~10 | ~4 |

**分批计划：**
- [ ] **F05b-1 (简单因子, ~30个)**: 多周期波动/涨跌幅/振幅/量比 — 复用现有算子
- [ ] **F05b-2 (中等因子, ~40个)**: 偏度/峰度/分位数/EMA交叉/威廉/CCI — 新增算子
- [ ] **F05b-3 (困难因子, ~33个)**: 动态相关性/市场相对收益/行业中性 — 需要面板数据
...
- [x] **条件选股/AI选股筛选功能不可用** ✅ 已修复 (2026-07-01) — 根因：YAML条件未包装+close字段缺失+K线源501。修复：条件包装+L4字段补全+数据源健康检测
