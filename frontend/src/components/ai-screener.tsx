...
/** v5.2 — AI 选股组件(从 /screener/page.tsx 提取,供 Tab 容器引用)
 *
 * 设计:
 *  - 自包含 — 所有 state/effect/handler 都在内部
 *  - 不含 SiteHeader — 由调用方(Tab 容器)统一提供
 */
...
import { IconSearch, IconBrain, IconBolt, IconStar, IconTrash, IconChartBar, IconUsers, IconCheck, IconX } from "@tabler/icons-react"
...
export function AIScreener() {
  ...
  const [includeGem, setIncludeGem] = useState(false)
  ...
  const fetchData = useCallback(async () => {
    try {
      ...
      const wlNorm: WatchItem[] = wlArr
        .map((w) => ({
          code: String(w.code || w.stock_code || ""),
          name: String(w.name || w.stock_name || ""),
          reason: w.reason,
          score: w.score,
          id: w.id,
        }))
