...
class TestAggregateTransactions:
    def test_returns_empty_for_no_transactions(self, _test_db_session):
    ...
    def test_aggregates_real_data(self, _test_db_session):
        import pytest
    ...
    def test_win_rate_calculation(self, _test_db_session):
        result = aggregate_transactions(user_id=1)
...
@pytest.mark.asyncio
async def test_generate_report_cold_start(_test_db_session):
    """Verify cold start behavior when insufficient transactions exist"""
    from services.review_service import generate_review_report

    result = await generate_review_report(user_id=99999)
    assert result["cold_start"] is True
    assert "至少需要" in result["summary"] and "笔交易" in result["summary"]
